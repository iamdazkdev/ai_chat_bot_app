...
class HomeController extends GetxController {
  RxList<CategorySuggestionModel> topCategoryList = <CategorySuggestionModel>[].obs;
  ...
  getNewCategory() {
    topCategoryList.add(CategorySuggestionModel(id: 0, name: 'AI Code', icon: 'assets/images/ai.png'));
    topCategoryList.add(CategorySuggestionModel(id: 0, name: 'Astrology', icon: 'assets/images/astrologyIcon.png'));
  }
  ...
  Future<dynamic> getCategory() async {
    try {
      final response = await http.get(Uri.parse(ApiServices.categories), headers: ApiServices.authHeader);
  ...
  Future<dynamic> getBanner() async {
    try {
      final response = await http.get(Uri.parse(ApiServices.banners), headers: ApiServices.authHeader);
  ...
  Future<dynamic> getHistoryProms() async {
    try {
      Map<String, dynamic> bodyParams = {'user_id': Preferences.getString(Preferences.userId), 'category_id': "0"};
      final response = await http.post(Uri.parse(ApiServices.getPromsHistory), headers: ApiServices.header, body: jsonEncode(bodyParams));
      log(response.request.toString());
      log(response.body);
      Map<String, dynamic> responseBody = json.decode(response.body);
  ...
  Future<dynamic> increaseLimit() async {
    ...
    try {
      ...
      final response = await http.post(Uri.parse(ApiServices.adsSeenLimitIncrease), headers: ApiServices.authHeader, body: jsonEncode(bodyParams));
      log(response.request.toString());
      log(response.body);
      Map<String, dynamic> responseBody = json.decode(response.body);
  ...
  Future<dynamic> deleteByIdProms({required HistoryData model}) async {
    ShowToastDialog.showLoader('Please wait'.tr);
    try {
      Map<String, dynamic> bodyParams = {'user_id': Preferences.getBoolean(Preferences.isLogin) ? Preferences.getString(Preferences.userId) : "", 'subject': model.subject.toString()};
      log(bodyParams.toString());
      final response = await http.post(Uri.parse(ApiServices.deletePromsHistory), headers: ApiServices.header, body: jsonEncode(bodyParams));
      log(response.request.toString());
      log(response.body);
      Map<String, dynamic> responseBody = json.decode(response.body);
